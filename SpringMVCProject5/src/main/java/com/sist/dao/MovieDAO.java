...
public class MovieDAO {
	private MongoClient mc; // mongodb Connection
	private DB db; // mongodb Database
	private DBCollection dbc; // mongodb table
	...
	public void movieInsert(MovieVO vo)
	{
		try {
			...
			obj.put("mno", vo.getMno()); // key, value.(map)
	...
	public List<MovieVO> movieListData(int page)
	{
		...
		{
			int rowsize=12;
			int skip=(rowsize*page)-rowsize;// 이번 페이지: 이전 페이지까지 나온 데이터들 개수(int skip)는 스킵하고 그 뒤의 데이터부터 노출
			
			DBCursor cursor=dbc.find().skip(skip).limit(rowsize); // DBCursor = ResultSet
	...
	public MovieVO movieDetailData(int mno)
	{
		...
		try {
			...
			// WHERE mno=10 // ex) mno=10 인 애 찾아라 
